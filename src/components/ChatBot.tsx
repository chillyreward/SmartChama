...
export function ChatBot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 0,
      from: 'bot',
      text: 'Hello! I am the SmartChama AI Assistant. I can help you with contributions, loans, credit scores, group management, and M-Pesa payments. What would you like to know?',
  ...
  async function handleSend(overrideText?: string) {
    ...
    try {
      ...
      const answer = data.reply || data.error || 'Sorry, I could not get a response. Please try again.'
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label="Open SmartChama AI Assistant">
        {open ? (
      ...
      {open && (
        <div
          ...
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ backgroundColor: '#22C55E' }}>
            ...
            <div className="flex-1 text-left">
              <p className="text-white font-semibold text-[15px] leading-tight">SmartChama Assistant</p>
              <p className="text-white/70 text-[12px] leading-none mt-0.5">Powered by AI · SmartChama only</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div
                  ...
                  <p className="text-[10px] mt-1" style={{ color: msg.from === 'user' ? 'rgba(255,255,255,0.7)' : 'var(--text-muted)' }}>
                    {msg.time}
                  </p>
                </div>
              </div>
            ...
            {typing && (
              <div className="flex justify-start items-center gap-2">
                ...
                <div className="rounded-2xl px-4 py-3" style={{ backgroundColor: 'var(--bg-subtle)' }}>
                  <div className="flex gap-1 items-center">
                    {[0, 1, 2].map(i => (
                      <div key={i} className="w-2 h-2 rounded-full bg-[#22C55E] animate-bounce"
          ...
          {/* Suggested questions */}
          <div className="px-3 py-2 flex gap-2 overflow-x-auto scrollbar-hide flex-shrink-0"
            ...
            {suggestedQuestions.map(q => (
              <button
                ...
                disabled={typing}
                className="flex-shrink-0 px-3 py-1.5 rounded-full text-[12px] font-medium whitespace-nowrap transition-colors hover:bg-[#22C55E] hover:text-white disabled:opacity-50"
                style={{
          ...
          <div className="flex items-center gap-2 px-3 py-3 flex-shrink-0"
            ...
            <input
              ...
              placeholder="Ask about SmartChama..."
